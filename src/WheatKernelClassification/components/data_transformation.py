...
class DataTransformation:
    ...
    def initialize_data_transformation(self,train_path,test_path):
        ...
        try:
            ...
            train_df = pd.read_csv(train_path, sep='\s+', header=None, names=Features)
            test_df = pd.read_csv(test_path, sep='\s+', header=None, names=Features)
            ...
            encoder = LabelEncoder().fit(train_df[target_column_name])
            #target_column_name = encoder.transform(test_df[target_column_name])

            input_feature_train_df = train_df.drop(columns=drop_columns, axis=1)
            target_feature_train_df = encoder.transform(train_df[target_column_name])

            input_feature_test_df = test_df.drop(columns = drop_columns, axis=1)
            target_feature_test_df = encoder.transform(test_df[target_column_name])
